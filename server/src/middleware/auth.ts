...
// Require admin or hotel_person role; if hotel_person, scope to assigned hotels only
export const requireHotelAccess = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  const token = req.cookies?.session;
  if (!token) {
    res.status(401).json({ error: "Unauthorized" });
    return;
  ...
  try {
    const payload = jwt.verify(token, JWT_SECRET) as JwtPayload;
    req.user = payload;
    if (!["admin", "hotel_person"].includes(payload.role)) {
      res.status(403).json({ error: "Forbidden: hotel access required" });
      return;
    }
    if (payload.role === "hotel_person") {
      const assignments = await HotelPersonAssignment.find({ userId: payload.id }).distinct("hotelId");
      req.scope = { hotelIds: assignments.map((id: any) => id.toString()) };
    } else {
      req.scope = {};
    }
    next();
...
export const scopeByRole = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  try {
    ...
    } else if (payload.role === "hotel_person") {
      const hotelAssignments = await HotelPersonAssignment.find({ userId: payload.id }).distinct("hotelId");
      req.scope = { hotelIds: hotelAssignments.map((id: any) => id.toString()) };
    } else {
      req.scope = {};
    }
